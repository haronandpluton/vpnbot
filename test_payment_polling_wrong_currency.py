...
async def main():
    ...
    telegram_id = int(f"599993{suffix[-6:]}")
    ...
    async with SessionLocal() as session:
        ...
        order = await order_service.create_order(
            ...
            username=f"wrong_currency_test_user_{suffix}",
            first_name="WrongCurrency",
            last_name="Tester",
            language_code="ru",
        )
        ...
        order.destination_address = f"wrong_currency_receiver_{suffix}"
        await session.commit()

        tx = NormalizedTransaction(
            txid=f"wrong_currency_txid_{suffix}",
            amount=Decimal("4.00"),
            currency="USDC",
            network="TRC20",
            ...
            provider="mock",
            raw_payload={
                "source": "wrong_currency_test",
                "expected_currency": "USDT",
                "actual_currency": "USDC",
                "network": "TRC20",
                "amount": "4.00",
            },
        )
        ...
        assert db_event.error_message == "wrong_currency"

        assert db_payment.status == PaymentStatus.INVALID
        assert db_payment.currency == "USDC"
        ...
        print("WRONG CURRENCY POLLING TEST PASSED")
